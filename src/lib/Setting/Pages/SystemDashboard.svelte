<script lang="ts">
    ...
    import { alertConfirm, alertMd, notifyError, notifySuccess, notifyWarning } from 'src/ts/alert'
    ...
    async function runPurgeOrphans() {
        ...
        try {
            ...
            // The server sweep is held back until its reference set covers our
            // KV/atomic/inlay assets. It answers 200 with disabled:true and
            // deletes nothing — reporting that as success would have the user
            // confirm an irreversible purge and then watch the count stay put.
            if (json?.disabled) {
                notifyWarning(language.storageOrphanDisabled(typeof json.reason === 'string' ? json.reason : ''))
                await loadStats()
                return
            }
...
{#if stats}
    ...
    <div class="border border-darkborderc bg-darkbg/40 rounded-md p-4 mb-4">
        ...
        <div class="flex items-center justify-between gap-3 mb-3">
            <div class="min-w-0">
                <div class="text-textcolor text-sm flex items-center gap-2 flex-wrap">
                    {language.storageOrphanAutoClean}
                    {#if !stats.orphan.available}
                        <ShBadge variant="secondary">{language.storageOrphanAutoCleanPending}</ShBadge>
                    {/if}
                </div>
                <div class="text-textcolor2 text-xs leading-relaxed">{language.storageOrphanAutoCleanDesc}</div>
            </div>
            <!-- The boot sweep runs through the same disabled server function, so
                 leaving this switch live would let the user arm a no-op. -->
            <ShSwitch bind:checked={DBState.db.nodeOnlyAutoCleanAssets} disabled={!stats.orphan.available} />
        </div>
