...
describe('NodeStorage authFetch transient retry', () => {
    test('normal partial and strict final saves share FIFO ordering, including after a failed save', async () => {
        const { encodeRisuSaveLegacy } = await import('./risuSave')
        vi.mocked(encodeRisuSaveLegacy).mockImplementation((chat: any) => new TextEncoder().encode(chat.data))
        let release!: (res: Response) => void
        const held = new Promise<Response>((resolve) => { release = resolve })
        const fetchMock = vi.fn().mockReturnValueOnce(held)
            .mockResolvedValueOnce(jsonResponse(200, { success: true, durable: true }))
        const { storage } = setUpStorage(fetchMock)
        const partial = storage.saveChatContent('char', 0, 'chat', { data: 'partial' })
        const failed = expect(partial).rejects.toThrow()
        const chat = { data: 'final' }
        const final = storage.saveChatContentStrict('char', 0, 'chat', chat)
        chat.data = 'later local edit'
        await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1))
        release(jsonResponse(400))
        await failed
        await expect(final).resolves.toMatchObject({ durable: true })
        expect(fetchMock).toHaveBeenCalledTimes(2)
        expect(new TextDecoder().decode(fetchMock.mock.calls[1][1].body)).toBe('final')
        expect(fetchMock.mock.calls[1][1].headers.get('x-strict-flush')).toBe('1')
    })
