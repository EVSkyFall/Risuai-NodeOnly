...
function makeHarness(runtimeId = 'host-runtime') {
    ...
    const deps: IllustrationV3BridgeDependencies = {
        ...
        createImageRevision: vi.fn(async () => structuredClone(rawJob)),
        getImageRevisionTarget: vi.fn(async () => ({ protocolVersion: 1 })),
        listImageRevisions: vi.fn(async () => ({ protocolVersion: 1, items: [], nextCursor: null })),
        restoreImageRevision: vi.fn(async () => ({ referenceId: 'ref' })),
        enqueueRevisionImage: vi.fn(async () => structuredClone(rawJob)),
        listImageReferences: vi.fn(async () => ({ protocolVersion: 1, items: [], nextCursor: null })),
