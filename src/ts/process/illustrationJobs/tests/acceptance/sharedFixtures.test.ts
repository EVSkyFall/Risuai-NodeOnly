...
describe('Gate 4d shared Core fixtures', () => {
    ...
    // 0.2.1 interim drafts, and the removed 0.2.0/0.2.1/0.2.2/0.2.3 releases — never
    // authorizes.
    test('pins the 0.2.5 JSON/poll stability rotation window and rejects every superseded digest', async () => {
        expect(PINNED_ILLUSTRATION_PLUGIN_DIGESTS).toEqual([
            PRODUCTION_PLUGIN.scriptSha256,
            PRODUCTION_PLUGIN.scriptSha256Next,
        ])
        expect(REJECTED_PLUGIN_SHA256S).toHaveLength(10)
