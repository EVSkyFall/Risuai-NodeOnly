...
const mocks = vi.hoisted(() => ({
    ...
    saveChatToServerStrict: vi.fn(async () => ({ success: true, durable: true })),
    fetchChatFromServer: vi.fn(async () => null),
...
vi.mock('src/ts/storage/chatStorage', () => ({
    ...
    saveChatToServerStrict: mocks.saveChatToServerStrict,
    fetchChatFromServer: mocks.fetchChatFromServer,
}))
vi.mock('src/ts/requestLog', () => ({ recordRequestLog: mocks.recordRequestLog }))
...
function setupServer(behavior: ServerBehavior) {
    ...
    const claimedJobs = new Map<string, string>()
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        if (claimMatch && method === 'POST') {
            const claimToken = JSON.parse(String(init?.body ?? '{}')).claimToken as string
            const owner = claimedJobs.get(claimMatch[1])
            const won = owner === undefined || owner === claimToken
            if (owner === undefined) claimedJobs.set(claimMatch[1], claimToken)
            if (behavior.claimResponseLostOnce && !claimResponseLost) {
                claimResponseLost = true
                throw new TypeError('claim response lost')
            }
            return new Response(JSON.stringify({ success: won }), { status: 200 })
        }
        if (claimMatch && method === 'GET') {
            const claimToken = new Headers(init?.headers).get('x-model-job-claim-token')
            return new Response(JSON.stringify({ owned: claimedJobs.get(claimMatch[1]) === claimToken }), { status: 200 })
        }
        if (claimMatch && method === 'DELETE') {
            const claimToken = new Headers(init?.headers).get('x-model-job-claim-token')
            const released = claimedJobs.get(claimMatch[1]) === claimToken
            if (released) claimedJobs.delete(claimMatch[1])
    ...
    const releases = () => calls.filter((c) => c.method === 'DELETE' && c.url.endsWith('/claim')).map((c) => c.url)
    return { calls, claims, releases }
}
...
beforeEach(() => {
    ...
    mocks.saveChatToServerStrict.mockReset()
    mocks.saveChatToServerStrict.mockResolvedValue({ success: true, durable: true })
    mocks.fetchChatFromServer.mockReset()
...
describe('recoverTerminalJob', () => {
    ...
    test('a lost claim response is confirmed by token before slot-in', async () => {
        const { recovery } = await loadModules()
        const chat = makeChat()
        mocks.db.characters = [makeChar(chat)]
        const { calls, claims } = setupServer({
            journals: { 'job-1': OPENAI_SSE },
            claimResponseLostOnce: true,
        })
...
describe('recovered chat is persisted', () => {
    test('the CAS winner persists the inserted message to the server', async () => {
        const { recovery } = await loadModules()
        const chat = makeChat()
        mocks.db.characters = [makeChar(chat)]
        const { claims } = setupServer({ journals: { 'job-1': OPENAI_SSE } })
        ...
        expect(mocks.saveChatToServerStrict).toHaveBeenCalledTimes(1)
        expect(mocks.saveChatToServerStrict).toHaveBeenCalledWith('cha-1', 0, 'chat-1', chat)
        expect(claims()).toEqual(['/api/model-jobs/job-1/claim'])
    })
    ...
    test('a filled partial message is saved too', async () => {
        ...
        expect(mocks.saveChatToServerStrict).toHaveBeenCalledTimes(1)
        expect((mocks.saveChatToServerStrict.mock.calls[0] as any)[3].message[0].data).toBe('Hello')
    })
    ...
    test('definite absence releases the claim and a retry saves without duplicating the log', async () => {
        ...
        const chat = makeChat()
        mocks.db.characters = [makeChar(chat)]
        mocks.saveChatToServerStrict
            .mockRejectedValueOnce(new Error('saveChatContent error: 507'))
            .mockResolvedValueOnce({ success: true, durable: true })
        mocks.fetchChatFromServer.mockResolvedValueOnce(null)
        const { claims, releases } = setupServer({ journals: { 'job-1': OPENAI_SSE } })
    ...
    test('ambiguous readback keeps the claim and does not log or retry the save', async () => {
        ...
        mocks.db.characters = [makeChar(chat)]
        mocks.saveChatToServerStrict.mockRejectedValue(new Error('durable ACK lost'))
        mocks.fetchChatFromServer.mockRejectedValue(new Error('readback unavailable'))
        const { claims, releases } = setupServer({ journals: { 'job-1': OPENAI_SSE } })
