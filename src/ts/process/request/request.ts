...
import {
    ...
    type AdapterReasoningPart, type AdapterStructuredOutput, type AdapterToolCall, type AdapterToolDef, type AdapterUsage,
...
import { shouldStageMainRequestSnapshot, stageMainRequestSnapshot } from "./requestReplay";
...
export async function requestChatData(arg:requestDataArgument, model:ModelModeExtended, abortSignal:AbortSignal=null):Promise<requestDataResponse> {
    ...
    for(let fallbackIndex=0;fallbackIndex<fallBackModels.length;fallbackIndex++){
        ...
        while(true){
            ...
            if(shouldStageMainRequestSnapshot(model, arg)){
                ...
                // Continue requests are intentionally staged too: their
                // response remains on the existing message chatId, and the
                // commit path aliases this generation's exact request bytes
                // back to that persisted message identity.
...
async function requestModelPreset(arg:RequestDataArgumentExtended, preset:ModelPreset, abortSignal:AbortSignal=null, mode:ModelModeExtended='model'):Promise<requestDataResponse> {
    ...
    const transportFetch = useServerJob
        ? makeJobFetch({
            ...
            onJobCreated: arg.realChatId ? arg.onJobCreated : undefined,
            streaming: resolvePresetStreaming(preset, arg),
