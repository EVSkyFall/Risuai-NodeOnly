...
// place. A load failure leaves only the descriptor — the plugin then sees the
...
export async function hydratePluginCharacterSnapshot<T extends AssetFields>(
    ...
    if (!snapshot.additionalAssetManifest) return snapshot
    delete snapshot.additionalAssets
    try {
...
export async function hydratePluginModuleSnapshot<T extends ModuleAssetFields>(
    ...
    if (!snapshot) return snapshot
    if (!snapshot.assetManifest) return snapshot
