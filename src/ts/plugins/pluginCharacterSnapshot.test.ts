...
describe('hydratePluginCharacterSnapshot', () => {
    test('failed loads and sync cache misses discard conflicting inline lists', async () => {
        const missing = { ...descriptor, id: 'batch2-missing' }
        loadAssetManifestItems.mockRejectedValue(new Error('unavailable'))
        const snapshot: any = { additionalAssets: [], additionalAssetManifest: missing }
        await hydratePluginCharacterSnapshot(snapshot)
        expect(snapshot).toEqual({ additionalAssetManifest: missing })
        expect(hydratePluginCharacterSnapshotSync({ additionalAssets: [], additionalAssetManifest: missing }))
            .toEqual({ additionalAssetManifest: missing })
        const module: any = { assets: [], assetManifest: missing }
        await hydratePluginModuleSnapshot(module)
        expect(module).toEqual({ assetManifest: missing })
    })
    ...
    test('hydrates the descriptor when a legacy snapshot also carries an inline list', async () => {
