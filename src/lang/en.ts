...
export const languageEnglish = {
    ...
    swipePrevTooltip: "Previous regenerated message (generates a new one when there are none)",
    swipeNextTooltip: "Next regenerated message (generates a new one at the end of the list)",
    ...
    memoryPresetNoSettings: "This preset has no editable memory settings.",
    ...
    showMemoryInSidebar: "Show Long-term Memory Binding in Sidebar",
    ...
    addBlankMessage: "Add blank message",
    ...
    // Points at this fork's own permission preset panel, which upstream has no
    // equivalent of: it puts the single permission back rather than wiping every
    // stored answer the way the reset does.
    pluginPermissionDenyGuide: "Permission request from plugin \"{}\" was denied. Denied permissions will not be asked again.\n\nOpen \"Permission presets\" for this plugin in [Settings] > [Plugins] to set this one back to \"Allow\" or \"Ask\". \"Reset permission responses\" clears every stored answer at once.",
    ...
    pluginStorageV2PreloadFailed: "Plugin storage could not be fully loaded. V2 plugins will still start with unavailable values treated as unreadable. Reload to retry loading storage.",
    ...
    pluginStorageBackendSave: "Plugin Storage",
    pluginStorageBackendSaveDesc: "Normal pluginStorage API · server-backed · included in backups",
    ...
    pluginStorageValueSearchLimited: (limit: string) => `Value search checks only the first ${limit} of each value.`,
    pluginStorageValueSearchDisabled: "Value search is disabled for Device DB because each record must be fully loaded before it can be searched.",
    pluginStorageValueSearchError: "Value search failed",
    ...
    pluginStoragePreviewTruncatedExact: (limit: string, total: string) => `Preview limited to ${limit}. Full size: ${total}. Editing is disabled to protect the stored value.`,
    pluginStoragePreviewTruncatedUnknown: (limit: string) => `Preview limited to ${limit}. Full size is at least this large. Editing is disabled to protect the stored value.`,
    pluginStorageOwner: "Origin",
    pluginStorageOwnerUnknown: "Unknown",
    pluginStorageSizeUnknown: "Size is only known once the entry is opened.",
    ...
    // Covers both reasons the scan can be off: the database is not in server
    // memory yet, and the sweep being held back until its reference set covers
    // every asset. Naming only the first sends the user refreshing forever.
    storageOrphanUnavailable:
        "Orphan media cannot be scanned right now — either the database is not in server memory yet, or the reference list is still incomplete. Try refreshing in a moment.",
    storageOrphanDisabled: (reason: string) =>
        reason
            ? `Orphan purging is still switched off (${reason}). Nothing was deleted, and it stays off until the reference list covers every asset.`
            : "Orphan purging is still switched off. Nothing was deleted, and it stays off until the reference list covers every asset.",
    storageOrphanAutoCleanPending: "Pending",
    ...
    updatePopupWaitingBackup: "Waiting for backup",
    updatePopupBackupFailedAbort: "The backup failed, so the update was not started.",
