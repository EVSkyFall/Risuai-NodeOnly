...
This NodeOnly fork keeps its existing `plugin-custom-storage/<encoded-key>.json` server KV namespace. The v1.11.2 integration adds metadata indexing and lazy reads without relocating existing values. Consequences:
...
- V2 / V2.1 plugins use a synchronous API, so every key is preloaded right before the first enabled V2 plugin runs. The preload is shared within the session. On a storage outage, hooks still start and one notification explains that some values may be unreadable.
...
- Native full and settings-only backups include custom/blob KV entries; the main DB field stays empty. Settings-only backups exclude chat-coupled atomic state. Full exports targeting upstream RisuAI embed custom values into the DB instead; that compatibility target retains its existing exclusions for NodeOnly blob/inlay namespaces.
- The "Provide all plugin data" setting is stored on the plugin entry and ignored by upstream RisuAI.
- Existing NodeOnly builds that use this custom KV namespace keep reading the same values. Restoring an older NodeOnly snapshot without a plugin companion preserves the current custom/blob values; a validated companion restores its custom values after a rescue copy is saved.
