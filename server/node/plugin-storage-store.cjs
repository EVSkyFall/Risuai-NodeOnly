...
// kv row `plugin-custom-storage/<base64url(key)>.json` holding UTF-8 `JSON.stringify(value)`.
...
const PREFIX = 'plugin-custom-storage/';
const MIGRATED_MARKER_KEY = '_meta/plugin-storage-migrated';
...
function decodeKey(kvKey) {
    const encoded = kvKey.startsWith(PREFIX) ? kvKey.slice(PREFIX.length, -'.json'.length) : kvKey;
    const base64 = encoded.replace(/-/g, '+').replace(/_/g, '/');
    return Buffer.from(base64, 'base64').toString('utf-8');
}

function kvKeyFor(pluginKey) {
    return `${PREFIX}${encodeKey(pluginKey)}.json`;
}
...
function createPluginStorageStore(deps) {
    ...
    function list() {
        ...
        for (const entry of kvListWithSizes(PREFIX)) {
            ...
            out.push({ key: decodeKey(entry.key), size: entry.size, updatedAt: deps.kvGetUpdatedAt?.(entry.key) ?? null });
        }
        return out;
    }
    ...
    function snapshotTo(snapshotId) {
        ...
        const entries = [];
        let marker = kvGet(MIGRATED_MARKER_KEY)?.toString('utf-8') ?? null;
        const now = Date.now();
        for (const row of kvListWithSizes(PREFIX)) {
            if (!row.key.endsWith('.json')) continue;
    ...
    // Absence is preservation; an explicit, validated empty map is replacement.
    function validateSnapshot(snapshotId) {
        const map = readSnapshotMap(snapshotId);
        if (!map) {
            const prefix = snapshotPrefixFor(snapshotId);
            const rows = kvListWithSizes(prefix);
            for (const row of rows) {
                const key = row.key.slice(prefix.length);
                if (key !== '__migrated__' && !/^[A-Za-z0-9_-]*(?:\.json)?$/.test(key)) throw new Error('Invalid legacy plugin snapshot key');
                const raw = kvGet(row.key);
                if (raw == null) throw new Error('Legacy plugin snapshot row missing');
                JSON.parse(Buffer.from(raw).toString('utf-8'));
            }
            return rows.length > 0;
        }
        if (map.v !== SNAPSHOT_MAP_VERSION) throw new Error('Unsupported plugin snapshot version');
        const keys = new Set();
        for (const entry of map.entries) {
            if (!Array.isArray(entry) || entry.length !== 2) throw new Error('Malformed plugin snapshot entry');
            const [enc, sha] = entry;
            const liveKey = typeof enc === 'string' && enc.endsWith('.json') ? enc : `${enc}.json`;
            if (typeof enc !== 'string' || !/^[A-Za-z0-9_-]*(?:\.json)?$/.test(enc) || keys.has(liveKey)
                || typeof sha !== 'string' || !/^[a-f0-9]{64}$/.test(sha)) {
                throw new Error('Invalid plugin snapshot key or hash');
            }
            keys.add(liveKey);
            const raw = kvGet(blobKeyFor(sha));
            if (raw == null || sha256(raw) !== sha) throw new Error('Plugin snapshot blob missing or corrupt');
            JSON.parse(Buffer.from(raw).toString('utf-8'));
        }
        return true;
    }
    ...
    function restoreFrom(snapshotId) {
        if (!validateSnapshot(snapshotId)) return false;
        const map = readSnapshotMap(snapshotId);
        return db.transaction(() => {
            kvDelPrefix(PREFIX);
            if (!map) {
                const prefix = snapshotPrefixFor(snapshotId);
                kvDel(MIGRATED_MARKER_KEY);
                for (const row of kvListWithSizes(prefix)) {
                    const key = row.key.slice(prefix.length);
                    const target = key === '__migrated__' ? MIGRATED_MARKER_KEY
                        : `${PREFIX}${key.endsWith('.json') ? key : `${key}.json`}`;
                    kvSet(target, kvGet(row.key));
                }
                return true;
            }
            const now = Date.now();
            for (const [enc, sha] of map.entries) {
                const info = stmtCopyRow.run(`${PREFIX}${enc.endsWith('.json') ? enc : `${enc}.json`}`, now, blobKeyFor(sha));
                if (info.changes !== 1) throw new Error('Plugin snapshot blob disappeared during restore');
            }
            if (typeof map.marker === 'string') kvSet(MIGRATED_MARKER_KEY, Buffer.from(map.marker, 'utf-8'));
            else kvDel(MIGRATED_MARKER_KEY);
            return true;
        })();
    }
    ...
    return {
        ...
        snapshotTo, validateSnapshot, restoreFrom, dropSnapshot, gcBlobs, snapshotBytes, snapshotLogicalBytes,
    };
}
...
module.exports = {
    ...
    validateSnapshot: (...a) => getDefaultStore().validateSnapshot(...a),
