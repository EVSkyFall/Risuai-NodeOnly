...
describe('model-jobs', () => {
    ...
    it('claim flow: unclaimed listing, claim, then excluded', async () => {
        ...
        const claimRes = await fetch(`${base}/api/model-jobs/${jobId}/claim`, {
            method: 'POST',
            headers: { 'risu-auth': AUTH_TOKEN, 'content-type': 'application/json' },
            body: JSON.stringify({ claimToken: 'claim-flow' }),
        })
    ...
    it('releases only a claimed recoverable terminal job so it can be claimed again', async () => {
        upstream.chunks = ['data: retryable\n\n']
        upstream.delayMs = 5
        upstream.hang = false
        ...
        const jobId = json.jobId as string
        await waitForStatus(base, jobId, ['done'])
        expect(store.claimJob(jobId, 'release-owner')).toEqual({ success: true })
        expect(store.claimJob(jobId, 'release-owner')).toEqual({ success: true })
        expect(store.claimJob(jobId, 'other-owner')).toEqual({ success: false })
        expect(store.ownsJobClaim(jobId, 'release-owner')).toBe(true)
        expect(store.releaseJobClaim(jobId, 'release-owner')).toEqual({ released: true })
        expect(store.getJob(jobId).claimed).toBe(false)
        expect(store.claimJob(jobId, 'release-owner-2')).toEqual({ success: true })
        expect(store.releaseJobClaim(jobId, 'wrong-owner')).toEqual({ released: false })
        expect(store.releaseJobClaim(jobId, 'release-owner-2')).toEqual({ released: true })
        expect(store.releaseJobClaim(jobId, 'release-owner-2')).toEqual({ released: false })
    })

    it('model-job claim is atomic: exactly one concurrent claimer wins', async () => {
        upstream.chunks = ['data: claim-race\n\n']
        upstream.delayMs = 5
        upstream.hang = false
        ...
        const claim = (claimToken: string) => fetch(`${base}/api/model-jobs/${jobId}/claim`, {
            method: 'POST',
            headers: { 'risu-auth': AUTH_TOKEN, 'content-type': 'application/json' },
            body: JSON.stringify({ claimToken }),
        }).then((res) => res.json())
        const results = await Promise.all([claim('claim-a'), claim('claim-b')])
    ...
    it('claiming a running job returns 409', async () => {
        ...
        const claimRes = await fetch(`${base}/api/model-jobs/${json.jobId}/claim`, {
            method: 'POST',
            headers: { 'risu-auth': AUTH_TOKEN, 'content-type': 'application/json' },
            body: JSON.stringify({ claimToken: 'running-claim' }),
        })
        ...
        expect(store.releaseJobClaim(json.jobId, 'running-claim')).toEqual({ released: false })
    })
    ...
    it('pending sends register (upsert per chat), list, and clear over HTTP', async () => {
        ...
        const del = await fetch(`${base}/api/pending-sends/chat-ps-1`, {
            method: 'DELETE',
            headers: { 'risu-auth': AUTH_TOKEN, 'content-type': 'application/json' },
            body: JSON.stringify({ generationId: 'gen-b' }),
        })
    ...
    it('restore-if-absent never overwrites a newer pending generation', async () => {
        store.registerPendingSend({ chatId: 'chat-restore-1', generationId: 'old' })
        expect(store.claimPendingSend('chat-restore-1', 'old')).toEqual({ claimed: true })
        expect(store.restorePendingSendIfAbsent({ chatId: 'chat-restore-1', generationId: 'old' }))
            .toEqual({ restored: true })
        expect(store.restorePendingSendIfAbsent({ chatId: 'chat-restore-1', generationId: 'older' }))
            .toEqual({ restored: false })
        store.registerPendingSend({ chatId: 'chat-restore-1', generationId: 'new' })
        expect(store.clearPendingSend('chat-restore-1', 'old')).toEqual({ cleared: false })
        expect(store.claimPendingSend('chat-restore-1', 'old')).toEqual({ claimed: false })
        expect(store.restorePendingSendIfAbsent({ chatId: 'chat-restore-1', generationId: 'old' }))
            .toEqual({ restored: false })
        expect(store.listPendingSends().find((row: any) => row.chatId === 'chat-restore-1')?.generationId)
            .toBe('new')
        store.clearPendingSend('chat-restore-1', 'new')
    })
