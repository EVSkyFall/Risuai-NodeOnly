...
describe('asset manifest migration compatibility layer', () => {
    it('detects coexistence in every owner and projects legacy reads descriptor-first', () => {
        const store = freshStore()
        const descriptor = store.putManifest('module', 'mod', [['a', 'asset', 'png']])
        const mixed = {
            modules: [{ id: 'mod', assets: [], assetManifest: descriptor }],
            characters: [{ chaId: 'char', additionalAssets: [], additionalAssetManifest: descriptor }],
            personas: [{ id: 'persona', embeddedModule: { assets: [], assetManifest: descriptor } }],
        }
        expect((migrationPkg as any).findAssetManifestConflicts(mixed)).toHaveLength(3)
        const projected = stripAssetManifests(mixed, store).db
        expect((migrationPkg as any).findAssetManifestConflicts(projected)).toEqual([])
        expect(projected.modules[0].assetManifest).toBe(descriptor)
        expect(projected.modules[0].assets).toBeUndefined()
        expect(mixed.modules[0].assets).toEqual([])
    })

    it('keeps legacy persona manifest ownership stable across reordering', () => {
        const store = freshStore()
        const persona = { name: 'Legacy', embeddedModule: { assets: [['a', 'assets/a.png', 'png']] } }
        const first = stripAssetManifests({ personas: [persona] }, store).db.personas[0]
        const reordered = stripAssetManifests({ personas: [{ name: 'Other' }, persona] }, store).db.personas[1]
        expect(reordered.embeddedModule.assetManifest).toEqual(first.embeddedModule.assetManifest)
        const identified = { ...persona, id: 'stable-persona' }
        const stable = stripAssetManifests({ personas: [identified] }, store).db.personas[0]
        const renamed = stripAssetManifests({ personas: [{ ...identified, name: 'Renamed' }] }, store).db.personas[0]
        expect(renamed.embeddedModule.assetManifest).toEqual(stable.embeddedModule.assetManifest)
    })
