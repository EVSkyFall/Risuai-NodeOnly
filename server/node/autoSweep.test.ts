...
describe('/api/db/assets/auto-sweep', () => {
    it('reports manual purge unavailable and counts native plugin backup bytes once', async () => {
        await seedReferencedDb()
        await writeKey('assets/orphan.png', 'orphan')
        const stats = async () => {
            const res = await fetch(`${base}/api/db/stats`, { headers: authHeaders() })
            expect(res.status).toBe(200)
            return res.json()
        }
        const before = await stats()
        expect(before.orphan).toMatchObject({ available: false, count: 0, totalSize: 0 })
        const value = JSON.stringify({ stored: 'fixture' })
        await writeKey(`plugin-custom-storage/${Buffer.from('fixture').toString('base64url')}.json`, value)
        const after = await stats()
        expect(after.estimatedBackupSize - before.estimatedBackupSize).toBe(Buffer.byteLength(value))
        const res = await fetch(`${base}/api/db/assets/purge-orphans`, { method: 'POST', headers: authHeaders() })
        expect(res.status).toBe(200)
        expect(await res.json()).toMatchObject({ ok: true, disabled: true, reason: 'Plugin reference coverage pending', deleted: 0 })
        expect(hasKey('assets/orphan.png')).toBe(true)
    })

    it('preserves assets of every age while plugin reference coverage is pending', async () => {
        await seedReferencedDb()
    ...
    it('also preserves remote rows when assets=false', async () => {
        ...
        expect(body.remotesDeleted).toBe(0)
        expect(hasKey('assets/old.png')).toBe(true)
        expect(hasKey('remotes/orphan.local.bin')).toBe(true)
        expect(hasKey('remotes/orphan.local.bin.meta')).toBe(true)
    })

    it('reports disabled without deleting from an empty reference set', async () => {
    ...
    it('stays disabled without touching corrupt manifests or their assets', async () => {
        await seedDb({
            characters: [{ chaId: 'keep', image: 'assets/avatar.png', chats: [] }],
    ...
    it('preserves stale/recent remotes and their metadata without inventing metadata', async () => {
        await seedReferencedDb()
        ...
        expect(hasKey('remotes/old.local.bin')).toBe(true)
        expect(hasKey('remotes/old.local.bin.meta')).toBe(true)
        ...
        expect(hasKey('remotes/no-meta.local.bin.meta')).toBe(false)
        expect(hasKey('remotes/orphan.local.bin.meta')).toBe(true)
    })
})
