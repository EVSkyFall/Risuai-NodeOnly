...
function createModelJobs(opts = {}) {
    ...
    try {
        db.exec(`ALTER TABLE model_jobs ADD COLUMN claim_token TEXT`);
    } catch { /* column already present */ }
    try {
    ...
    const stmtClaim = db.prepare(`
        UPDATE model_jobs SET claimed = 1, claim_token = ?
        WHERE id = ? AND claimed = 0 AND status IN ('done', 'failed')
    `);
    const stmtReleaseClaim = db.prepare(`
        UPDATE model_jobs SET claimed = 0, claim_token = NULL
        WHERE id = ? AND claim_token = ? AND claimed = 1 AND status IN ('done', 'failed')
    `);
    ...
    `);
    const stmtPendingRestoreIfAbsent = db.prepare(`
        INSERT INTO pending_sends (chat_id, generation_id, created_at) VALUES (?, ?, ?)
        ON CONFLICT(chat_id) DO NOTHING
    `);
    const stmtPendingDeleteOwned = db.prepare(`
        DELETE FROM pending_sends WHERE chat_id = ? AND generation_id IS ?
    `);
    ...
    function restorePendingSendIfAbsent(arg) {
        const chatId = typeof arg?.chatId === 'string' ? arg.chatId : '';
        if (!chatId) return { error: 'chatId is required', httpStatus: 400 };
        const info = stmtPendingRestoreIfAbsent.run(
            chatId,
            typeof arg.generationId === 'string' ? arg.generationId : null,
            Date.now()
        );
    ...
    function clearPendingSend(chatId, generationId) {
        const info = stmtPendingDeleteOwned.run(chatId, generationId ?? null);
    ...
    function claimPendingSend(chatId, generationId) {
        const info = stmtPendingDeleteOwned.run(chatId, generationId ?? null);
    ...
    function claimJob(jobId, claimToken) {
        if (typeof claimToken !== 'string' || claimToken.length === 0 || claimToken.length > 200) {
            return { error: 'claimToken is required', httpStatus: 400 };
        }
        const row = stmtGet.get(jobId);
        if (!row) return { error: 'Job not found', httpStatus: 404 };
        if (row.status !== 'done' && row.status !== 'failed') {
            return { error: 'Job is not recoverable', httpStatus: 409 };
        }
        if (row.claimed) return { success: row.claim_token === claimToken };
        const result = stmtClaim.run(claimToken, jobId);
        return { success: result.changes > 0 };
    }

    function ownsJobClaim(jobId, claimToken) {
        if (typeof claimToken !== 'string' || claimToken.length === 0) return false;
        const row = stmtGet.get(jobId);
        return !!row && row.claimed === 1 && row.claim_token === claimToken;
    }

    function releaseJobClaim(jobId, claimToken) {
        const result = stmtReleaseClaim.run(jobId, claimToken);
    ...
    function registerRoutes(app, { auth }) {
        ...
        app.post('/api/model-jobs/:id/claim', async (req, res) => {
            if (!await auth(req, res)) return;
            const result = claimJob(req.params.id, req.body?.claimToken);
            if (result.error) {
        ...
        app.get('/api/model-jobs/:id/claim', async (req, res) => {
            if (!await auth(req, res)) return;
            res.send({ owned: ownsJobClaim(req.params.id, req.get('x-model-job-claim-token')) });
        });

        app.delete('/api/model-jobs/:id/claim', async (req, res) => {
            if (!await auth(req, res)) return;
            res.send(releaseJobClaim(req.params.id, req.get('x-model-job-claim-token')));
        });
        ...
        app.delete('/api/pending-sends/:chatId', async (req, res) => {
            if (!await auth(req, res)) return;
            res.send(clearPendingSend(req.params.chatId, req.body?.generationId));
        });

        app.post('/api/pending-sends/:chatId/claim', async (req, res) => {
            if (!await auth(req, res)) return;
            res.send(claimPendingSend(req.params.chatId, req.body?.generationId));
        });

        app.post('/api/pending-sends/:chatId/restore', async (req, res) => {
            if (!await auth(req, res)) return;
            const result = restorePendingSendIfAbsent({
