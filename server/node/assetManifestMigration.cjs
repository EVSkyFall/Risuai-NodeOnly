...
function fallbackOwnerId(prefix, name, index) {
    return `${prefix}-${crypto.createHash('sha256')
        .update(index === undefined ? String(name || '') : `${String(name || '')}\0${index}`)
        .digest('hex')
        .slice(0, 24)}`;
}
...
function personaOwnerId(persona) {
    return String(persona?.id || persona?.personaId || fallbackOwnerId('persona', persona?.name));
}
...
function stripAssetManifests(dbObj, store, { activate = true } = {}) {
    ...
    if (Array.isArray(dbObj.characters)) {
        out.characters = dbObj.characters.map((character, index) => {
            if (character?.additionalAssetManifest) {
...
function findAssetManifestConflicts(dbObj) {
    const conflicts = [];
    const check = (kind, list, ownerIdOf, fields) => {
        if (!Array.isArray(list)) return;
        list.forEach((owner, index) => {
            const [array, descriptor] = fields(owner);
            if (Array.isArray(array) && descriptor) conflicts.push({ kind, ownerId: ownerIdOf(owner, index), index });
        });
    };
    check('module', dbObj?.modules, moduleOwnerId, (m) => [m?.assets, m?.assetManifest]);
    check('character', dbObj?.characters, characterOwnerId, (c) => [c?.additionalAssets, c?.additionalAssetManifest]);
    check('persona', dbObj?.personas, personaOwnerId, (p) => [p?.embeddedModule?.assets, p?.embeddedModule?.assetManifest]);
    return conflicts;
}
