...
describe('key encoding', () => {
    test('matches persistentKv base64url and round-trips special keys', () => {
        ...
        for (const k of ['a::chunk:v1:x', '한글 키', '__proto__', 'x/y+z=', ''.padEnd(300, 'k')]) {
            expect(decodeKey(`${PREFIX}${encodeKey(k)}.json`)).toBe(k)
        }
...
describe('migrateFromDb (unit)', () => {
    ...
    test('removeAll clears values while retaining the no-remigration marker', () => {
        const { store } = memoryKv()
    ...
    test('cold migration preserves existing KV even after marker loss; explicit writes still merge', () => {
        const { store } = memoryKv()
        let snapshots = 0
        const snap = { createSnapshot: () => { snapshots++ } }
        // Lost/absent marker never permits overwriting an existing KV row.
        store.set('a', 'stale-kv')
        expect(store.migrateFromDb({ pluginCustomStorage: { a: 'db-1', b: 'db-1' } }, { ...snap, kvWinsOnRemigration: true }))
            .toMatchObject({ migrated: true, keys: 1, skipped: 1 })
        expect(snapshots).toBe(1)
        expect(store.get('a')).toBe('stale-kv')
...
describe('content-addressed snapshots (unit)', () => {
    ...
    test('snapshotTo/restoreFrom reproduce the exact set; one map row, no row copies', () => {
        ...
        expect(map.entries.sort()).toEqual([[encodeKey('__proto__') + '.json', sha('P')], [encodeKey('a') + '.json', sha('A')]].sort())
        ...
        // A legacy snapshot without a companion preserves current custom values.
        expect(store.restoreFrom('200')).toBe(false)
        expect(store.readAll()).toEqual(JSON.parse('{"__proto__":"P","a":"A"}'))
        expect(store.isMigrated()).toBe(true)
        store.removeAll()
        ...
        store.set('z', 1)
        store.snapshotTo('100')
        expect(mapOf(deps, '100').entries).toEqual([[encodeKey('z') + '.json', sha(1)]])
        expect(mapOf(deps, '100').marker).not.toBeNull()
    ...
    test('a snapshot whose blob went missing fails restore atomically', () => {
        ...
        expect(() => run()).toThrow(/blob.*missing/i)
    ...
    test('legacy row-copy layout still restores, sizes and drops', () => {
        ...
        db.prepare(`INSERT INTO kv (key, value, updated_at)
                    SELECT ? || substr(key, ?), value, updated_at FROM kv WHERE key LIKE 'plugin-custom-storage/%'`)
            .run(store.snapshotPrefixFor('old'), PREFIX.length + 1)
        deps.kvSet(store.snapshotPrefixFor('old') + '__migrated__', deps.kvGet(MIGRATED_MARKER_KEY))
...
describe('server: import → migrate → index → export', () => {
    ...
    test('imported plugin storage is split into kv and the served DB field is empty', async () => {
        ...
        const hex = Buffer.from(`${PREFIX}${encodeKey('libra::item::chunk:v1:0')}.json`).toString('hex')
        ...
        expect(exported.some(e => e.name.startsWith(PREFIX))).toBe(true)
        const dbEntry = exported.find(e => e.name === 'database.risudat')!
        const exportedDb = await utils.decodeRisuSave(dbEntry.data) as any
        expect(exportedDb.pluginCustomStorage ?? {}).toEqual({})
        const upstream = await client.fetch('/api/backup/export?target=upstream')
        const upstreamEntries = decodeBackup(Buffer.from(await upstream.arrayBuffer()))
        expect(upstreamEntries.some(e => e.name.startsWith(PREFIX))).toBe(false)
        const upstreamDb = await utils.decodeRisuSave(upstreamEntries.find(e => e.name === 'database.risudat')!.data) as any
        expect(upstreamDb.pluginCustomStorage).toEqual(seeded)
        ...
        await client.fetch('/api/write', {
            method: 'POST',
            headers: { 'file-path': Buffer.from(`${PREFIX}${encodeKey('stale')}.json`).toString('hex'), 'content-type': 'application/octet-stream' },
    ...
    test('a legacy save folder is migrated on first cold load', async () => {
        ...
        const index = await readIndex(client)
        expect(index.migrated).toBe(true)
        expect(index.entries).toMatchObject([{ key: 'boot', size: Buffer.byteLength('"value"') }])
    })
})
