...
async function writePluginKey(client: RisuClient, key: string, value: unknown) {
    await writeKv(client, `${PREFIX}${encodeKey(key)}.json`, Buffer.from(JSON.stringify(value)))
}

async function readPluginKey(client: RisuClient, key: string) {
    const res = await client.fetch('/api/read', { headers: { 'file-path': hex(`${PREFIX}${encodeKey(key)}.json`) } })
...
describe('F1: snapshots carry plugin storage', () => {
    test('old snapshots preserve live custom/blob values; valid companions replace custom after rescue', async () => {
        const { client } = await boot({ POCKETRISU_BACKUP_INTERVAL_MS: '0' })
        expect((await client.importBackup(backupWith({ a: 'A' }))).ok).toBe(true)
        const oldSnapshot = 'database/dbbackup-1.bin'
        await writeKv(client, oldSnapshot, dbBlob({}))
        const blobKey = 'plugin-blob-storage/YmxvYg.json'
        await writeKv(client, blobKey, Buffer.from('"blob-data"'))
        await writePluginKey(client, 'b', 'B')
        await tick()
        await writeKv(client, DB_KEY, dbBlob({}))
        const currentSnapshot = (await snapshots(client))[0]
        expect(currentSnapshot.key).not.toBe(oldSnapshot)

        await writePluginKey(client, 'a', 'new')
        await writePluginKey(client, 'c', 'C')
        await restoreSnapshot(client, oldSnapshot)
        expect((await indexKeys(client)).keys).toEqual(['a', 'b', 'c'])
        expect(await readPluginKey(client, 'a')).toBe('new')
        expect(await readKvJson(client, blobKey)).toBe('blob-data')

        await restoreSnapshot(client, currentSnapshot.key)
    ...
    test('R2: a raw /api/remove of a snapshot key drops its map row and unique blobs', async () => {
        ...
        await client.fetch('/api/remove', { headers: { 'file-path': hex(`${PREFIX}${encodeKey('b')}.json`) } })
        await tick()
    ...
    test('R4: a failed snapshot does not advance the backup cooldown', async () => {
        const preload = fileURLToPath(new URL('./compat/helpers/fail-snapshot-preload.cjs', import.meta.url)).replace(/\\/g, '/')
...
describe('F3: save-folder import', () => {
    test('preserves plugin-free imports and replaces actual custom payloads after rescue', async () => {
        const { client } = await boot()
        ...
        const emptyImport = await client.fetch('/api/migrate/save-folder/upload', {
            method: 'POST', headers: { 'content-type': 'application/zip' },
            body: new Uint8Array(saveFolderZip({ [DB_KEY]: dbBlob({}) })),
        })
        expect(emptyImport.status).toBe(200)
        expect((await indexKeys(client)).keys).toEqual(['old', 'old2'])
