...
const PLUGIN_CUSTOM_KEY = 'plugin-custom-storage/settings.json'
const PLUGIN_BLOB_KEY = 'plugin-blob-storage/blob.json'
const PLUGIN_ATOMIC_KEY = 'plugin_atomic/v1.json'
...
function createRichSeed(): Buffer {
  ...
  return encodeBackup([
    ...
    { name: PLUGIN_CUSTOM_KEY, data: Buffer.from('{"theme":"dark"}') },
    { name: PLUGIN_BLOB_KEY, data: Buffer.from([1, 2, 3, 4]) },
    {
      name: PLUGIN_ATOMIC_KEY,
      data: Buffer.from(JSON.stringify({ rows: [{
        key: 'p:33333333-3333-4333-8333-333333333333:test/atomic', revision: 1, value: Buffer.from('state').toString('base64'),
        deleted: 0, updatedAt: 1, changeSeq: 1,
      }] })),
    },
  ])
}
...
describe('settings-only export', () => {
  ...
  test('keeps plugin settings KV but excludes chat-coupled plugin atomic state', async () => {
    const client = await seededServer()
    const settingsNames = decodeBackup((await exportSettingsOnly(client)).bin).map(e => e.name)

    expect(settingsNames).toEqual(expect.arrayContaining([PLUGIN_CUSTOM_KEY, PLUGIN_BLOB_KEY]))
    expect(settingsNames).not.toContain(PLUGIN_ATOMIC_KEY)

    const fullNames = decodeBackup(await client.exportBackup()).map(e => e.name)
    expect(fullNames).toEqual(expect.arrayContaining([PLUGIN_CUSTOM_KEY, PLUGIN_BLOB_KEY, PLUGIN_ATOMIC_KEY]))
  })
